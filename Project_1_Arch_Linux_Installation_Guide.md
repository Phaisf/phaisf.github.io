...
## 4.1 Problems
Although I did not have many issues, there were a few glaring ones that caused me a lot of time to remediate or get past.  
- For some reason, every time I restarted my VM while I was still in the preparation phase, I had to re-partition my disks until eventually it just stopped disappearing on startup. Never found the reason for it. Another problem with the disks was mounting them. I had to keep mounting on every startup up until I booted into the Desktop Environment. 
- I also had forgotten to install a boot loader for my VM before restarting and attempting to boot into the DE, which left me extremely confused at first until I realized I skipped over installing it. 
- Booting into the VM also was taking forever, which I found was due to only allocating 1 CPU core to the VM, so allocating 2 fixed the slow boot time.
- The last problem I had was my alias' not saving after restarting, which I only found out about during my initial recording, which was just a problem of forgetting to save the file after exiting. 
